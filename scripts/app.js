// This file is generated by push_app_js.py
// It builds buttons for each sport and fetches JSON from the API in-browser.
...
// On page load, build the button bar and fetch “all” by default
$(document).ready(() => {
...
// When a filter button is clicked, re-fetch for that sport
...
async function fetchAndDisplay(sport) {
  // If “all”, fetch every sport except “all” itself
  ...
  const columns = Object.keys(allRows[0]).map(col => {
    return { title: col.replace(/_/g, " ").toUpperCase(), data: col };
  });
